...
if($_SERVER["REQUEST_METHOD"]==="POST")
{
 ...
 try
 {
  ...
  if($errors)
  {
   $_SESSION["passwordResetErrors"]=$errors;
   ...
   if(!($result["userStatus"]==="deleted"))
   {
    if(!($result["userStatus"]==="pending"))
    {
     if($result["userStatus"]==="activated")
     {
      ...
      {
       header("Location: ../../pages/passwordreset.php?forgotAPassword=failedToSendAMail");
       exit();
      }
     }
     else
     {
      header("Location: ../../pages/passwordreset.php?forgotAPassword=somethingWentWrong");
      exit();
     }
    }
    else
    {
     header("Location: ../../pages/passwordreset.php?forgotAPassword=notVerified");
     exit();
    }
   }
   else
   {
    header("Location: ../../pages/passwordreset.php?forgotAPassword=emailIsDeleted");
    exit();
   }
  }
  else
  {
   header("Location: ../../pages/passwordreset.php?forgotAPassword=failed");
   exit();
  }
 }
...
function passwordchangeEmailVerification($email,$firstname,$lastname,$token,$mail)
{
 ...
 $url="http://127.0.0.1/touristAgency/code/pages/passwordchange.php?token=";
 $HTMLContent=
  '
   <div style="background:#18191f;">
    ...
    <div style="background:#3d3f4f;">
     <div style="display:flex;justify-content:center;">
      <h3 style="color:#e9e9e9;padding:1rem;">Hi '.htmlspecialchars($firstname).' '.htmlspecialchars($lastname).',you asked for change password email.</h3>
     </div>
     <br>
     <div style="display:flex;justify-content:center;">
      <p style="color:#e9e9e9;">Please, enter a page for changing your password with the link given below.</p>
     </div>
     <br>
     <div style="display:flex;justify-content:center;">
      <a href="'.$url.$token.'&email='.$email.'" style="color:gold;">Change password link</a>
     </div>
     <br>
    </div>
    <br>
